...
import (
	...
	boshlogfake "github.com/cloudfoundry/bosh-utils/logger/loggerfakes"
)
				...
				Context("and swap size is non-zero", func() {
					It("creates swap equal to specified amount", func() {
						...
						Expect(partitioner.PartitionPartitions).To(Equal([]boshdisk.Partition{
							...
							{
								SizeInBytes: diskSizeInBytes - (2048 * 4096),
								Type:        boshdisk.PartitionTypeLinux,
								SectorInfo: &boshdisk.PartitionSectorInfo{
									Start:         16512,
									SizeInSectors: 16256,
					...
					It("it does not return any sectorInfo even if the first partition has enough sectors", func() {
						var desiredSwapSize = diskSizeInBytes
						act = func() error {
							return platform.SetupEphemeralDiskWithPath("/dev/xvda", &desiredSwapSize)
						}
						partitioner.GetDeviceSizeInBytesSizes["/dev/xvda"] = diskSizeInBytes
						...
						Expect(errorMsgTag).To(Equal("linuxPlatform"))
						Expect(sectorPartitionError).To(ContainSubstring(`Calculating sector information for partition: 'linux': No more sectors remaining for this partition`))
					})
				})
